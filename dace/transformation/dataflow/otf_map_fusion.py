...
class OnTheFlyMapFusion(transformation.Transformation):
    ...
    def can_be_applied(graph, candidate, expr_index, sdfg, strict=False):
        ...
        # Check that locally consumed buffers all have the same range.
        ranges = set()
        for buffer in buffers:
            edge = graph.out_edges(buffer)[0]
    ...
    def apply(self, sdfg):
        ...
        locally_consumed_buffer = set()
        globally_consumed_buffer = set()
        for buffer in buffers:
            out_edge = graph.out_edges(buffer)[0]
            path = graph.memlet_path(out_edge)
            locally_consumed_buffer.add(path[path.index(out_edge) + 1].data.subset)
            globally_consumed_buffer.add(out_edge.data.subset)
        assert len(locally_consumed_buffer) == 1
        assert len(globally_consumed_buffer) == 1
        locally_consumed_buffer = locally_consumed_buffer.pop() # the range that is consumed in the second map
        globally_consumed_buffer = globally_consumed_buffer.pop() # the range that is consumed in the second map
        ...
        # for edge in graph.out_edges(map1_entry):
        ...
        for buffer in buffers:
            in_edge = graph.in_edges(buffer)[0]
            in_edge.data.subset.ranges = [(0, upper - lower, stride) for lower, upper, stride in in_edge.data.subset.ranges]
            out_edge = graph.out_edges(buffer)[0]
            out_edge.data.subset.ranges = [(0, upper - lower, stride) for lower, upper, stride in out_edge.data.subset.ranges]
